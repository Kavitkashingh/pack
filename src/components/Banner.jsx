...
const Banner = () => {
    ...
    return (
        <div className=' bg-slate-300 w-[100%] pt-4 flex flex-wrap justify-center items-center mx-auto'>
          <div className='w-[100%]  mx-auto relative'>
           <div className='flex flex-row pl-4 pb-8'>
            <div className='w-[300px] my-auto xs:hidden '>
                <div className='bg-[#d9d9d9] h-[87vh] md-lg:h-[103vh] rounded-xl overflow-hidden px-3'>
                    ...
                    {categorys?.map((category, index) => (
                            <li key={index} className='flex items-center px-4 gap-2 m-2 p-2 hover:bg-[#ff5a5a] hover:font-bold hover:text-white rounded-full'>
            ...
            <div className='w-[74%] md:w-3/4 lg:w-4/5 ml-3 overflow-hidden mx-auto'>
                {/* Carousel and Slider */}
                <div className='mb-3'>
                    <Carousel autoPlay={true} infinite={true} arrows={true} showDots={true} responsive={responsiveCarousel}>
                        {[1, 2, 3, 4].map((num, index) => (
                            <Link key={index} to='#' className='block w-full'>
                                <img src={`http://localhost:3000/images/banner/${num}.jpg`} alt="" className='object-cover rounded-2xl w-full h-[340px]' />
                            </Link>
                        ))}
                    </Carousel>
                </div>
                ...
                <div className="overflow-hidden">
                    <Slider {...settings}>
                        {[ imgs,img,  imga].map((image, index) => (
                            <div key={index} >
                                <img src={image} alt="Slide" className='rounded-xl object-cover h-[150px] '/>
                            </div>
                        ))}
                    </Slider>
                </div>
